...
<div class="table-bg">
    <el-table :data="goodslist">
        ...
        <el-table-column label="规格名" align="center">
            <template slot-scope="scope">
                <el-tag v-for="(item,index) in specslist" :key="index" v-if="item.id==scope.row.specsid" type="success">{{ item.specsname }}</el-tag>
            </template>
        </el-table-column>
    ...
    <el-pagination background @size-change="set_size" @current-change="set_page" :current-page="page" :page-sizes="[10, 20, 30, 40]" :page-size="size" layout="total, sizes, prev, pager, next, jumper" :total="total"></el-pagination>
</div>
</template>
